...
class CustomBatchNormManualFunction(torch.autograd.Function):
  ...
  def forward(ctx, input, gamma, beta, eps=1e-5):
    ...
    ctx.eps = eps
    n_var = 1/torch.sqrt(var+eps)
    ctx.save_for_backward(x_hat, var, gamma)
    return out
  ...
  def backward(ctx, grad_output):
    ...
    x_hat, var, gamma = ctx.saved_tensors
    eps = ctx.eps
    grad_input = grad_gamma = grad_beta = None
    n_batch = grad_output.shape[0]

    if ctx.needs_input_grad[0]:
        first_part = gamma/(n_batch * torch.sqrt(var + eps))
        second_part = n_batch * grad_output - grad_output.sum(0) - x_hat * ((x_hat * grad_output).sum(0))
        grad_input = first_part * second_part
